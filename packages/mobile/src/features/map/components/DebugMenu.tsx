...
const DebugMenu = () => {
  ...
  return (
    <>
      <button
        onClick={() => setOpen((v) => !v)}
        className="absolute top-16 right-4 z-20 bg-white rounded-full p-2 shadow-lg hover:bg-gray-100 active:scale-95 transition-transform"
      >
        <Bug className="size-5 text-yellow-500" />
      </button>

      {open && (
        <div className="absolute top-28 right-4 z-20 bg-black/85 text-white rounded-xl p-3 text-xs w-52 flex flex-col gap-2 shadow-xl">
          ...
          {debugPosition ? (
            <div className="flex flex-col gap-1">
              <div className="bg-white/10 rounded-lg px-2 py-1.5">
                <p className="text-white/50 text-[10px]">Simulated position</p>
              ...
              <button
                ...
                className="text-red-400 text-[10px] text-left px-1"
              >
                Clear simulation
              </button>
            </div>
          ...
          {userPosition && (
            <div className="bg-white/5 rounded-lg px-2 py-1.5 flex flex-col gap-0.5">
              <p className="text-white/50 text-[10px]">Active position</p>
